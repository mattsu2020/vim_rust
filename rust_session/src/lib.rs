...
/// Errors that can occur when handling session or viminfo files.
#[derive(Debug)]
...
pub fn load_session(path: &str) -> Result<(u32, String), SessionError> {
    let content = std::fs::read_to_string(path)?;
    let mut lines = content.lines();
    let ver_line = lines.next().ok_or(SessionError::Parse)?;
    let version = ver_line
        .strip_prefix("# vim session version ")
        .ok_or(SessionError::Parse)?
        .parse::<u32>()
        .map_err(|_| SessionError::Parse)?;
    Ok((version, lines.collect::<Vec<_>>().join("\n")))
}

/// Parse the version from the contents of a viminfo file.
pub fn parse_viminfo_version(contents: &str) -> Result<u32, SessionError> {
    for line in contents.lines() {
        if let Some(rest) = line.strip_prefix("# This viminfo file was generated by Vim ") {
            let version_str = rest.split_whitespace().next().ok_or(SessionError::Parse)?;
...
pub extern "C" fn rs_load_session(path: *const c_char, out_version: *mut u32) -> *mut c_char {
    ...
    if let Ok(p) = c_ptr_to_str(path) {
        if let Ok((version, data)) = load_session(p) {
            unsafe {
                if !out_version.is_null() {
                    *out_version = version;
                }
...
pub extern "C" fn rs_parse_viminfo(path: *const c_char) -> u32 {
    ...
    match std::fs::read_to_string(p) {
        Ok(s) => parse_viminfo_version(&s).unwrap_or(0),
        Err(_) => 0,
    }
}
...
mod tests {
    ...
    fn parse_viminfo_versions() {
        ...
        assert_eq!(parse_viminfo_version(new).unwrap(), 9);
        assert_eq!(parse_viminfo_version(old).unwrap(), 8);
    }

    #[test]
    fn parse_viminfo_invalid() {
        assert!(parse_viminfo_version("invalid").is_err());
    }
    ...
    #[test]
    fn load_session_parse_error() {
        let path = "Xbad_session.vim";
        fs::write(path, "no header\nbody").unwrap();
        assert!(matches!(load_session(path), Err(SessionError::Parse)));
        fs::remove_file(path).unwrap();
    }

    #[test]
    fn ffi_load_session_error() {
        let cpath = CString::new("Xbad_ffi_session.vim").unwrap();
        fs::write("Xbad_ffi_session.vim", "oops").unwrap();
        let mut ver = 123u32;
        let ptr = rs_load_session(cpath.as_ptr(), &mut ver as *mut u32);
        assert!(ptr.is_null());
        assert_eq!(ver, 0);
        fs::remove_file("Xbad_ffi_session.vim").unwrap();
    }
    ...
    #[test]
    fn viminfo_invalid() {
        fs::write("Xbad_viminfo", "garbage").unwrap();
        assert_eq!(
            rs_parse_viminfo(CString::new("Xbad_viminfo").unwrap().as_ptr()),
            0
        );
        fs::remove_file("Xbad_viminfo").unwrap();
    }
}
