...
# endif
    ...
    void (*encode_fn)(cryptstate_T *state,
                      char_u *from,
                      size_t len,
                      char_u *to,
                      int last);
    void (*decode_fn)(cryptstate_T *state,
                      char_u *from,
                      size_t len,
                      char_u *to,
                      int last);
    ...
    long (*encode_buffer_fn)(cryptstate_T *state,
                             char_u *from,
                             size_t len,
                             char_u **newptr,
                             int last);
    long (*decode_buffer_fn)(cryptstate_T *state,
                             char_u *from,
                             size_t len,
                             char_u **newptr,
                             int last);
    ...
    void (*encode_inplace_fn)(cryptstate_T *state,
                              char_u *p1,
                              size_t len,
                              char_u *p2,
                              int last);
    void (*decode_inplace_fn)(cryptstate_T *state,
                              char_u *p1,
                              size_t len,
                              char_u *p2,
                              int last);
...
static int
crypt_sodium_init_(cryptstate_T *state, char_u *key, crypt_arg_T *arg);
static long crypt_sodium_buffer_decode(cryptstate_T *state,
                                       char_u *from,
                                       size_t len,
                                       char_u **buf_out,
                                       int last);
static long crypt_sodium_buffer_encode(cryptstate_T *state,
                                       char_u *from,
                                       size_t len,
                                       char_u **buf_out,
                                       int last);
...
static struct
{
    ...
    { "crypto_secretstream_xchacha20poly1305_init_push",
      (SODIUM_PROC *)&dll_crypto_secretstream_xchacha20poly1305_init_push },
...
sodium_runtime_link_init(int verbose)
{
    ...
    {
        if (verbose)
            semsg(_(e_could_not_load_library_str_str), libname,
                  load_dll_error());
        return FAIL;
    }
    ...
    {
        if ((*sodium_funcname_table[i].ptr = symbol_from_dll(
                 hsodium, sodium_funcname_table[i].name)) == NULL)
        {
            close_dll(hsodium);
            hsodium = NULL;
            if (verbose)
                semsg(_(e_could_not_load_library_function_str),
                      sodium_funcname_table[i].name);
            return FAIL;
        }
    }
    return OK;
...
{
    return CRYPT_MAGIC_LEN + cryptmethods[method_nr].salt_len +
           cryptmethods[method_nr].seed_len + cryptmethods[method_nr].add_len;
}
 ...
 */
cryptstate_T *
crypt_create(int method_nr, char_u *key, crypt_arg_T *crypt_arg)
{
 ...
 */
cryptstate_T *
crypt_create_from_header(int method_nr, char_u *key, char_u *header)
{
    crypt_arg_T arg;
    ...
    if (arg.cat_add_len > 0)
        arg.cat_add =
            header + CRYPT_MAGIC_LEN + arg.cat_salt_len + arg.cat_seed_len;
...
crypt_create_from_file(FILE *fp, char_u *key)
{
    ...
    if (header_len > CRYPT_MAGIC_LEN &&
        fread(buffer + CRYPT_MAGIC_LEN, header_len - CRYPT_MAGIC_LEN, 1, fp) !=
            1)
    {
        vim_free(buffer);
        return NULL;
 ...
 */
cryptstate_T *
crypt_create_for_writing(int method_nr,
                         char_u *key,
                         char_u **header,
                         int *header_len)
{
 ...
 */
long
crypt_encode_alloc(cryptstate_T *state,
                   char_u *from,
                   size_t len,
                   char_u **newptr,
                   int last)
 ...
 */
long
crypt_decode_alloc(cryptstate_T *state,
                   char_u *ptr,
                   long len,
                   char_u **newptr,
                   int last)
 ...
 */
void
crypt_encode(cryptstate_T *state,
             char_u *from,
             size_t len,
             char_u *to,
             int last)
 ...
 */
void
crypt_encode_inplace(cryptstate_T *state, char_u *buf, size_t len, int last)
{
 ...
 */
void
crypt_decode_inplace(cryptstate_T *state, char_u *buf, size_t len, int last)
{
...
crypt_free_key(char_u *key)
{
    ...
    static void *(*volatile vim_memset_safe)(void *s, int c, size_t n) = memset;
    if (key != NULL)
    {
# ifdef FEAT_SODIUM
        if (sodium_init() >= 0)
              ...
              int twice) // Ask for the key twice.
{
    ...
    for (round = 0;; ++round)
    {
        cmdline_star = TRUE;
        cmdline_row = msg_row;
        p1 = getcmdline_prompt(NUL,
                               round == 0
                                   ? (char_u *)_("Enter encryption key: ")
                                   : (char_u *)_("Enter same key again: "),
                               0, EXPAND_NOTHING, NULL);
        cmdline_star = FALSE;
...
static int
crypt_sodium_init_(cryptstate_T *state UNUSED,
                   char_u *key UNUSED,
...
# ifdef FEAT_SODIUM
        ...
        state->method_nr == CRYPT_M_SOD)
    {
        ...
        if (state->method_nr == CRYPT_M_SOD2 && arg->cat_add != NULL)
        {
            ...
            memcpy(arg->cat_add, buffer,
                   sizeof(opslimit) + sizeof(memlimit) + sizeof(alg));
        }
    }
    else
    {
        ...
        char_u *p = buffer;
        vim_memset(buffer, 0, 20);
        int size = sizeof(opslimit) + sizeof(memlimit) + sizeof(alg);
...
#  ifdef FEAT_EVAL
        crypt_sodium_report_hash_params(
            opslimit, crypto_pwhash_OPSLIMIT_INTERACTIVE, (size_t)memlimit,
            crypto_pwhash_MEMLIMIT_INTERACTIVE, alg, crypto_pwhash_ALG_DEFAULT);
#  endif
 ...
 */
static long
crypt_sodium_buffer_encode(cryptstate_T *state UNUSED,
                           char_u *from UNUSED,
                           size_t len UNUSED,
...
# ifdef FEAT_SODIUM
    ...
    *buf_out = alloc_clear(length);
    ...
    if (first)
    {
        crypto_secretstream_xchacha20poly1305_init_push(&sod_st->state, ptr,
                                                        sod_st->key);
        ptr += crypto_secretstream_xchacha20poly1305_HEADERBYTES;
    }

    crypto_secretstream_xchacha20poly1305_push(&sod_st->state, ptr, &out_len,
                                               from, len, NULL, 0, tag);
 ...
 */
static long
crypt_sodium_buffer_decode(cryptstate_T *state UNUSED,
                           char_u *from UNUSED,
                           size_t len UNUSED,
                           char_u **buf_out UNUSED,
                           int last UNUSED)
...
# ifdef FEAT_SODIUM
    ...
    if (sod_st->count == 0 && state->method_nr == CRYPT_M_SOD &&
        len > WRITEBUFSIZE + crypto_secretstream_xchacha20poly1305_HEADERBYTES +
    ...
    *buf_out = alloc_clear(len);
    ...
    if (crypto_secretstream_xchacha20poly1305_pull(
            &sod_st->state, *buf_out, &out_len, &tag, from, len, NULL, 0) != 0)
    {
...
static void
crypt_sodium_report_hash_params(unsigned long long opslimit,
                                unsigned long long ops_def,
                                size_t memlimit,
                                size_t mem_def,
                                int alg,
