...
// Minimal loader backed by the Rust implementation.  The real Vim code keeps
// lots of data in a slang_T structure; for the purposes of these tests we only
// need a non-NULL pointer when the dictionary was loaded successfully.
static slang_T dummy_slang;

static slang_T *
spell_load_file(char_u *fname, char_u *lang UNUSED, void *dummy UNUSED, int silent UNUSED)
{
    if (rs_spell_load_dict((const char *)fname))
        return &dummy_slang;
    return NULL;
}
 ...
 */
    static void
find_word(matchinf_T *mip, int mode UNUSED)
{
    if (rs_spell_check((const char *)mip->mi_word))
        mip->mi_result = SP_OK;
    else
        mip->mi_result = SP_BAD;
    mip->mi_end = mip->mi_word + STRLEN(mip->mi_word);
}
 ...
 */
    static void
find_prefix(matchinf_T *mip UNUSED, int mode UNUSED)
{
    (void)mip;
    (void)mode;
}
