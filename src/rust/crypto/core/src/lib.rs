...
fn sha256_hex(data: &[u8], salt: &[u8]) -> String {
    let mut all = Vec::with_capacity(data.len() + salt.len());
...
fn sha256_start_rust(ctx: &mut context_sha256_T) {
    ...
    ctx.data = Box::into_raw(vec);
}

fn sha256_update_rust(ctx: &mut context_sha256_T, input: &[u8]) {
    if ctx.data.is_null() {
        sha256_start_rust(ctx);
    }
    unsafe {
        (*ctx.data).extend_from_slice(input);
    }
}
...
fn sha256_finish_rust(ctx: &mut context_sha256_T, digest: &mut [u8; 32]) {
    ...
    unsafe {
        let vec = Box::from_raw(ctx.data);
        *digest = sha256_digest(&vec);
    }
    ctx.data = std::ptr::null_mut();
}
...
pub extern "C" fn sha256_bytes(
    ...
    unsafe {
        ...
        let mut all = Vec::with_capacity(data.len() + salt_slice.len());
        all.extend_from_slice(data);
        all.extend_from_slice(salt_slice);
        let out = sha256_digest(&all);
...
pub extern "C" fn sha2_seed(
    ...
    unsafe {
        ...
        let sha256sum = sha256_digest(&random_data);
...
mod tests {
    ...
    fn sha256_bytes_abc() {
        let data = b"abc";
        let out = sha256_bytes(data.as_ptr() as *mut u8, data.len() as c_int, std::ptr::null_mut(), 0);
        let c_str = unsafe { CStr::from_ptr(out as *const i8) };
