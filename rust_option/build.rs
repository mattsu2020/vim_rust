...
fn main() {
    ...
    for line in content.lines() {
        ...
        if let Some(caps) = re.captures(t) {
            ...
            let kind = if flags.contains("P_BOOL") {
                "OptType::Bool"
            ...
            if seen.insert(var.clone()) {
                entries.push((name, short, kind.to_string()));
            }
        } else if let Some(caps) = term_re.captures(t) {
            let name = caps.get(1).unwrap().as_str().to_string();
            let var = variant_name(&name);
            if seen.insert(var.clone()) {
                entries.push((name, String::new(), "OptType::String".to_string()));
            }
        }
    }
    ...
    for (name, short, kind) in &entries {
        out.push_str(&format!(
